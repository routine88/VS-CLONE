...
keeping filenames and dimensions consistent.
...
placeholder data to production sprites.

For teams that prefer to generate quick visual stand-ins, the JSON can be fed
into a build step that emits PNGs or sprite sheets. The repository omits binary
image data so that the asset layout remains review-friendly on GitHub while the
manifest still captures the exact canvas requirements.
