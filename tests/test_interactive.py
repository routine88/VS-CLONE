...
def test_spawn_enemy_positions_respect_lane():
    ...
    def spawn_for(lane: EnemyLane) -> float:
        ...
        engine._activate_enemy(template, encounter_tag="wave")  # type: ignore[attr-defined]
...
def test_wave_and_miniboss_encounters_emit_feedback():
    engine = ArcadeEngine(spawn_interval=0.2, target_duration=90.0)

    wave_enemy = Enemy(name="Wave Foe", health=12, damage=3, speed=1.0, lane=EnemyLane.GROUND)
    wave_descriptor = WaveDescriptor(phase=1, wave_index=0, enemies=[wave_enemy])
    miniboss_enemy = Enemy(
        name="Grave Titan",
        health=80,
        damage=10,
        speed=0.5,
    ...
    encounters = iter(
        [
            Encounter(kind="wave", wave=wave_descriptor),
            Encounter(kind="miniboss", miniboss=miniboss_enemy, relic_reward="Sunstone"),
        ]
    )

    def fake_next_encounter(self: GameState) -> Encounter:
        try:
            return next(encounters)
        except StopIteration:
            return Encounter(kind="wave", wave=wave_descriptor)

    engine.state.next_encounter = types.MethodType(fake_next_encounter, engine.state)
    engine._encounter_timer = 0.0  # type: ignore[attr-defined]

    frame = engine.step(0.2, InputFrame())
    assert any(enemy.template.name == "Wave Foe" for enemy in frame.enemies)

    engine._pending_enemies.clear()  # type: ignore[attr-defined]
    engine._encounter_blocked = False  # type: ignore[attr-defined]
    engine._encounter_timer = 0.0  # type: ignore[attr-defined]

    miniboss_frame = engine.step(0.2, InputFrame())
    assert engine._active_miniboss is not None  # type: ignore[attr-defined]
    assert any("Miniboss" in message for message in miniboss_frame.messages)
    assert "run.miniboss_warning" in miniboss_frame.audio_events

    engine._reward_enemy(engine._active_miniboss)  # type: ignore[attr-defined]
    post_frame = engine.step(0.2, InputFrame())
    assert any("Relic acquired" in message for message in post_frame.messages)
    assert "run.relic_acquired" in post_frame.audio_events


def test_final_encounter_gates_victory():
    engine = ArcadeEngine(spawn_interval=0.2, target_duration=8.0)

    def fake_next_encounter(self: GameState) -> Encounter:
        thrall = Enemy(name="Thrall", health=6, damage=2, speed=0.6, lane=EnemyLane.GROUND)
        wave = WaveDescriptor(phase=self.current_phase, wave_index=0, enemies=[thrall])
        return Encounter(kind="wave", wave=wave)

    boss = Enemy(name="Twilight Tyrant", health=40, damage=12, speed=0.4, lane=EnemyLane.GROUND)
    final_encounter = Encounter(kind="final_boss", boss_phases=[boss])

    def fake_final_encounter(self: GameState) -> Encounter:
        return final_encounter

    engine.state.next_encounter = types.MethodType(fake_next_encounter, engine.state)
    engine.state.final_encounter = types.MethodType(fake_final_encounter, engine.state)

    frame = None
    for _ in range(40):
        frame = engine.step(0.3, InputFrame())
        if engine._final_encounter_triggered:  # type: ignore[attr-defined]
            break

    assert frame is not None
    assert engine._final_encounter_triggered  # type: ignore[attr-defined]
    assert "run.final_boss_warning" in frame.audio_events
    assert not frame.survived

    boss_enemy = next(
        enemy for enemy in engine._enemies if enemy.encounter_tag == "final_boss"
    )  # type: ignore[attr-defined]
    engine._reward_enemy(boss_enemy)  # type: ignore[attr-defined]
    victory_frame = engine.step(0.1, InputFrame())

    assert victory_frame.survived
    assert "run.final_boss_defeated" in victory_frame.audio_events
    assert any("Dawn" in message for message in victory_frame.messages)
